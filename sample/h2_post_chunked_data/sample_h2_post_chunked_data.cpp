...
int main() {
  ...
  // Send headers frame
  flags = make_frame_header_flags(mh2c::hf_flag::END_HEADERS);
  stream_id = 1u;
  ...
  for (size_t i = 0; i < body_chunks.size(); ++i) {
    const mh2c::fh_flags_t data_flags{
        i + 1 == body_chunks.size()
            ? make_frame_header_flags(mh2c::df_flag::END_STREAM)
            : mh2c::fh_flags_t{0u}};
    const mh2c::data_frame df{data_flags, stream_id, body_chunks[i]};
